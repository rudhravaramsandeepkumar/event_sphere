...
class EventInputs(db.Model):
    __bind_key__ = 'db'

    id = db.Column(db.Integer, Sequence('event_inputs_sequence'), unique=True, nullable=False, primary_key=True)
    event_id = db.Column(db.String(255),nullable=False)
    label_type = db.Column(db.String(255))
    label_name = db.Column(db.String(255))
    label_length = db.Column(db.String(255))
    label_mandatory = db.Column(db.String(255))

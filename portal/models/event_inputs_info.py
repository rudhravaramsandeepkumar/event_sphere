...
class EventInputsInfo(db.Model):
    __bind_key__ = 'db'

    id = db.Column(db.Integer, Sequence('event_inputs_info_sequence'), unique=True, nullable=False, primary_key=True)
    event_id = db.Column(db.String(255),nullable=False)
    candidate_id = db.Column(db.String(255), nullable=False)
    event_inputs_id = db.Column(db.String(255))
    event_inputs_answer = db.Column(db.String(255))
